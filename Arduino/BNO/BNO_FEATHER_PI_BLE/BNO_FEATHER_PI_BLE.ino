...
int32_t resetServiceId;
int32_t charid_reset;
...
void initBLE(void) {
  ...
  /* Add new service for reset msgs */
  Serial.println(F("Adding the Reset Service definition (UUID = 0x180D): "));
  success = ble.sendCommandWithIntReply( F("AT+GATTADDSERVICE=UUID128=00-11-00-11-33-44-55-66-77-88-AA-BB-CC-DD-EE-FF"), &resetServiceId);
  if (! success) {
    error(F("Could not add reset service"));
  }

   /* Add the reset characteristic */
  /* Chars ID for Measurement should be 1 */
  Serial.println(F("Adding the reset characteristic (UUID = 0x2A37): "));
  success = ble.sendCommandWithIntReply( F("AT+GATTADDCHAR=UUID128=02-11-88-11-22-33-44-55-66-77-AA-BB-CC-DD-EE-FF,PROPERTIES=0x10,MIN_LEN=1,MAX_LEN=17,VALUE=\"\""), &charid_reset);
  if (! success) {
    error(F("Could not add reset characteristic"));
  }
  ...
  /* Set callbacks */
  ble.setConnectCallback(connectedBLE);
  ble.setDisconnectCallback(disconnectedBLE);
  ble.setBleGattRxCallback(charid_reset, BleGattRX);
...
////RECEIVE MSGs OVER BLE FROM RPI////

//Bluetooth client connects to feather
void connectedBLE(void){
  Serial.println(F("Connected!"));
}

//Bluetooth client disconnects from feather
void disconnectedBLE(void){
  Serial.println(F("Disconnected!"));
}

//Bluetooth device writes on GATT characteristic
